...
- Treat the current baseline as a stable local frontend: the base app shell, first-level pages, core Xiangqi loop, result/replay flow, and minimum side-mode shells are already in place.
...
- Prefer local playable flows over static mockups: home, lobby, match, game, result, replay, and mode shells should remain connected.
...
- Use `xiangqi-rules-engine` for board state, legal moves, check/checkmate, AI moves, undo, replay, and game-result data.
